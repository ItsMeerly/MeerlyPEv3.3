-- ============================================================
-- PE v3.3 — Stable (NEW UX + Macro + Floating Memory Stats)
...
-- wired after UI exists
local setTransparency

-----------------------
-- GLOBAL SINGLETONS & STATE (single source of truth)
-----------------------
...
-- for hotkeys to update UI toggles
...
-- Floating memory stats UI state (OFF by default)
...
-----------------------
local memoryGui -- floating memory GUI (separate ScreenGui)
...
local pageNames = { "Automation", "Macro", "Performance", "XP", "Boss", "Misc", "Console", "Themes", "Help" }
...
-----------------------
-- Layout-frame tagging (fixes grey backdrops)
-----------------------
...
-----------------------
-- Wire transparency safely (skip layout frames)
-----------------------
...
-- Returns: button, setState(v, silent), getState()
...
---------------------------------------------------------------
-- ANTI-AFK LOOP (Space every 10 minutes)
---------------------------------------------------------------
task.spawn(function()
    local interval = 600
...
do
    ...
    local function recalcXP()
        ...
        local pot2x = tonumber(inputs["2x Potions (10m)"].Text) or 0
        local pot3x = tonumber(inputs["3x Potions (10m)"].Text) or 0

        local totalSeconds = 3600
        local sec3x = math.min(pot3x * 600, totalSeconds)
        local sec2x = math.min(pot2x * 600, totalSeconds - sec3x)
        local sec1x = totalSeconds - sec3x - sec2x

        local xp3x = baseXPhr * 3 * (sec3x / totalSeconds)
        local xp2x = baseXPhr * 2 * (sec2x / totalSeconds)
        local xp1x = baseXPhr * (sec1x / totalSeconds)

        local avgXPhr = xp3x + xp2x + xp1x
        local remainingXP = math.max(0, 100 - curXP)

        local hoursTo100 = (avgXPhr > 0) and (remainingXP / avgXPhr) or math.huge

        r1.Text = string.format("XP/hr: %.2f%% (Avg: %.2f%% w/ potions)", baseXPhr, avgXPhr)
        if hoursTo100 == math.huge then
            r2.Text = "Time to 100%: --"
        else
            r2.Text = string.format("Time to 100%%: %.2f hrs (%.1f mins)", hoursTo100, hoursTo100 * 60)
        end
...
do
    ...
    -- Extra longevity toggles (optional but useful)
    ...
    task.spawn(function()
        while running do
            ...
            if now - lastMemoryLog >= memoryLogInterval then
                lastMemoryLog = now
                local luaM = gcinfo()
                local totalMem = safeTotalMemMb()

                if totalMem then
                    log("System", string.format("Memory: Lua %.1f MB | Total %.1f MB", (luaM / 1024), totalMem))
                else
                    log("System", string.format("Memory: Lua %.1f MB", (luaM / 1024)))
                end
            end
        end
    end)
...
-- ============================================================
-- HELP PAGE (includes all hotkeys)
-- ============================================================
do
